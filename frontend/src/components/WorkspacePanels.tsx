...
function TraceabilityPanel({ traceability, onNavigate }: WorkspacePanelProps) {
  ...
  return (
    <div data-testid="workspace-traceability">
      ...
      <p className="mt-6 text-xs text-[#6b8095]">{t('analysis.workspace.traceability.matrixRows', { count: trace.matrix.length })}</p>
      <div className="mt-2 max-h-[42rem] overflow-auto rounded-xl border border-[#d7e3ee] bg-white">
        <table className="w-full min-w-[1100px] border-collapse text-left text-xs">
          <thead className="sticky top-0 z-10 bg-[#edf4fa] text-[#526a82]"><tr>{['review', 'role', 'finding', 'requirement', 'version', 'test'].map((key) => <th className="border-b border-[#d7e3ee] px-3 py-3 font-semibold" key={key}>{t(`analysis.workspace.traceability.matrix.${key}`)}</th>)}</tr></thead>
